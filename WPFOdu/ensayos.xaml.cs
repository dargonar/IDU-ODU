...
namespace dcf001
{
	public partial class ensayos
	{
        ...
        private void btnEliminarFallas_Click(object sender, RoutedEventArgs e)
        {
          ...
          //if (!WPFiDU.BL.ManagerUsuarios.sfUser.IsInRole(Role.Administrador))
          //{
        ...
        private void btnEliminarRegistro_Click(object sender, RoutedEventArgs e)
        {
          ...
          try
          {
            ...
            logger.InfoFormat("btnEliminar_Click(). Ensayo eliminado desde BBDD: Id=[{0}]; Marca=[{1}]; Modelo=[{2}]; NroSerie=[{3}]; OF=[{4}]; FechaEnsayo=[{5}]"
                  , ens.ID.ToString(), ens.Marca, ens.Modelo, ens.Serie, ens.OrdenFabricacion, ens.Fecha.ToString());             
        ...
        {
          if (!WPFiDU.BL.ManagerUsuarios.sfUser.IsInRole(Role.Operador))
          {
            excepcion exc = new excepcion("Seguridad", "Usted no está autorizado a acceder a esta sección.");
            exc.ShowDialog();
            exc = null;
        ...
        private void btnVer_Click(object sender, RoutedEventArgs e)
        {
          if (!WPFiDU.BL.ManagerUsuarios.sfUser.IsInRole(Role.Operador))
          {
            ...
            try
            {
                ...
                detallesensayoodu formulariodetallesodu = new detallesensayoodu(oEnsayo);
        ...
        private void btnReimprimirCaja_Click(object sender, RoutedEventArgs e)
        {
          ...
          //try
            ...
            //catch (Exception ex)
            //{
            //    logger.ErrorFormat("btnReimprimirCaja_Click(object sender, RoutedEventArgs e):: Message:'{0}', StackTrace:'{1}', InnerException:'{2}' "
            //   , ex.Message
            //   , ex.StackTrace
