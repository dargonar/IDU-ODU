    ...
    class OPCWrapper
    {
        ...
        public void Write(ParametrosEnsayo parametros)
        {
          string[] tagnames; double[] valores;
            ...
            PLCODU oPLCODU = new PLCODU();
            oPLCODU.GetParametrosAEscribir(parametros, out tagnamesList, out itemValuesList);
            ...
            for(int i=0;i<rItm.Length;i++)
            {
                ...
                {
                  logger.ErrorFormat(" OPC Tester: AddItems - '{0}' failed", tagnamesList[i]);              
                  theGrp.Remove(true); 
                  theSrv.Disconnect(); 
                  return;
                }                                      
