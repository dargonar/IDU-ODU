...
namespace iDU.BL
{
    class EtiquetaManager : Manager
    {
      ...
      private Bitmap GenerarBmp(Etiqueta etiqueta)
      {
        ...
        */
        ...
        Bitmap MapaDeImagen = new Bitmap(500, 742);
          ...
          foreach (EtiquetaItem item in etiqueta.Componentes)
          {
            switch (item.Tipo)
            {
              case 0:
                ...
                break;
              case 2:
                // DibujarStringVariable(MapaDeImagen,item);
                DibujarString(gr, item);
                break;
              case 4:
        ...
        using( SolidBrush brocha = new SolidBrush(System.Drawing.Color.Black) )
        {
          ...
          using( Font fuente = new Font(item.Font, item.Size,estilo) )
            gr.DrawString(item.ParamValue
              , fuente
              , brocha
              ,(float)item.Xinicial/10
              ,(float)item.Yinicial/10);
        }           
      }
      ...
      private void DibujarImagen(Graphics gr, EtiquetaItem item)
        {
          ...
          gr.DrawImage(imagen, 
            (float)item.Xinicial / 10.0f  
            , (float)item.Yinicial / 10.0f
            , (float)(item.Xfinal / 10.0f - item.Xinicial / 10.0f)
            , (float)(item.Yfinal / 10.0f - item.Yinicial / 10.0f));
      ...
      private void DibujarLinea(Graphics gr, EtiquetaItem item)
      {
        using (System.Drawing.Pen Lapiz = new System.Drawing.Pen(System.Drawing.Color.Black))
        {
          gr.DrawLine(Lapiz, 
            (float)item.Xinicial / 10, 
            (float)item.Yinicial / 10, 
            (float)item.Xfinal / 10, 
            (float)item.Yfinal / 10);
        }
      }
      ...
      public Bitmap DibujarGrilla()
      {
        Bitmap MapaDeImagen = new Bitmap(1000, 1000);
          ...
          for (int i = 0; i < MapaDeImagen.Height; i += 19)
          {
          ...
          for (int i = 0; i < MapaDeImagen.Width; i += 19)
          {
