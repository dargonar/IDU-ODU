     ...
     //   public abstract void ObtenerEtiquetasDBAnterior();
        ...
        public CaracteristicasTecnicas ObtenerCaracteristicaTecnicaDeModelo(Modelo m)
        {
            using (MySqlConnection conn = ConectarBaseDeDatos())
            {
                ...
                if (m.EsIdu)
                    sql += "where caracteristicastecnicasequipos_nombre = '" + m.Referencia + "' and (not caracteristicastecnicasequipos_idparametros_idu is null);";
                else
                    sql += "where caracteristicastecnicasequipos_nombre = '" + m.Referencia + "' and (caracteristicastecnicasequipos_idparametros_idu is null);";
        ...
        public List<Dictionary<string, string>> ListFallasControladas()
        {
          ...
          using (MySqlConnection conn = ConectarBaseDeDatos())
          {
            ...
            string sql = "SELECT numerodefalla, descripcion FROM fallasidu where es_falla_controlada = 1;";
            MySqlCommand cmd = new MySqlCommand(sql, conn);
            MySqlDataReader reader = cmd.ExecuteReader();
